 ...
 * @author steve
 */
public class NaviTaskActivity extends Activity{
	/**
	 *日志标识
	 */
	private static final String TAG="NaviTaskActivity";
	/**
	 *导航任务的ListView对象
	 */
...
//	};
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.nav_list_adpater);
		mBDCommManager=BDCommManager.getInstance(this.getApplicationContext());
		/*1.从数据库中查询所有的短信数据,如果数据库没有数据则发送指令请求最新插入的数据*/
		operation=new BDInstructionNavOperation(this);
	}
	
	@Override
	public void onStart(){
		super.onStart();
		listView=(ListView)this.findViewById(R.id.nav_list_listview);
		noNavTastPrompt=(TextView)this.findViewById(R.id.nav_message_prompt);
        ...
        if(navs.size()>0){
			/*2.把数据转换成List*/
			adapter=new NaviTaskAdapter(this, navs);
			listView.setAdapter(adapter);
			listView.setOnItemClickListener(new OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view,int position, long id){
					final BDInstructionNav bdInstructionNav=navs.get(position);
					AlertDialog.Builder builder=new AlertDialog.Builder(NaviTaskActivity.this);
			...
			listView.setOnItemLongClickListener(new OnItemLongClickListener(){
				public boolean onItemLongClick(AdapterView<?> arg0, View arg1,final int index, long arg3){
					AlertDialog.Builder alert=new AlertDialog.Builder(NaviTaskActivity.this);
					alert.setTitle("删除短信");
					...
					alert.setPositiveButton("删除", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface arg0, int index1) {
							...
							if(istrue){
							  navs.remove(index);
							  adapter.notifyDataSetChanged();
						      Toast.makeText(NaviTaskActivity.this, "删除指令导航成功!", Toast.LENGTH_SHORT).show();  
							}else{
							   Toast.makeText(NaviTaskActivity.this,"删除指令导航失败!", Toast.LENGTH_SHORT).show();   
							}
						}
					});
					alert.setNeutralButton("全部删除", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							if(istrue){
							   navs.clear();
							   adapter.notifyDataSetChanged();
							   Toast.makeText(NaviTaskActivity.this, "全部删除指令导航成功!", Toast.LENGTH_SHORT).show();
							}else{
							   Toast.makeText(NaviTaskActivity.this, "全部删除指令导航失败!", Toast.LENGTH_SHORT).show();
							}
						}
					});
	...
	public class NaviTaskAdapter extends BaseAdapter {
		...
		public View getView(final int position, View contentView, ViewGroup parent) {
			...
			if(newNavId==nav.getRowId()){
				Bitmap bm=BitmapFactory.decodeResource(mContext.getResources(), R.drawable.message_not_read);
				viewHolder.imageView.setImageBitmap(bm);
			}else{
				Bitmap bm=BitmapFactory.decodeResource(mContext.getResources(), R.drawable.message_received_box);
				viewHolder.imageView.setImageBitmap(bm);
			}
			...
			viewHolder.naviBtn.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View arg0){
					...
					notificationIntent.setAction("com.mapabc.android.activity.NaviStudioActivity");
					Bundle mBundle=new Bundle();
					mBundle.putLong("NAVI_ID", nav.getRowId());
