 ...
 * @author steve
 */
public class LineTaskActivity extends Activity{
	/**
	 *日志标识
	 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_nav_line);
		mBDCommManager=BDCommManager.getInstance(mContext.getApplicationContext());
	...
	public void onStart() {
		super.onStart();
		listView=(ListView)this.findViewById(R.id.nav_line_listview);
		noLinePrompt=(TextView)this.findViewById(R.id.no_nav_line_prompt);
		/*1.从数据库中查询所有的短信数据,如果数据库没有数据则发送指令请求最新插入的数据*/
		navs = operation.getNavLineList();
		mLineTaskAdapter=new LineTaskAdapter(mContext, navs);
		if(navs.size()>1000){
