...
public class SMSReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (info != null) {
            ...
            if (msg.length() > 1) {
                ...
                flag = mMessageArray[0];
            }
            if (msg.startsWith("$BDNAL")) {
                parseBDNAL(context, info, msg);
            } else if (msg.startsWith("$BDNAC")) {
                parseBDNAC(context, info, msg);
            } else if (msg.startsWith("$ZZHT")) {
                parseZZTH(context, info, msg);
            } else if (msg.startsWith("CMGS")) {
                msg = msg.substring(4);
                storeMsg4G(context , info , msg);
            } else {
                /*位置报告*/
                if (flag == (byte) 0xA0) {
                    parseLocationReport(context, info, msg, mMessageArray);
                } else if (flag == (byte) 0xA2) {
                    Intent mIntent = new Intent();
                    mIntent.setClassName(mContext, "com.novsky.map.DBLocationService");
                    mContext.startService(mIntent);
                } else if (msg.startsWith("F1")) {//1个字节
                    parseFriendLocation(context, info, msg);
                }
                else {
    ...
    private void parseFriendLocation(Context context, BDMessageInfo info, String msg) {
        // 友邻位置
        try {
            //发送时间
            ...
            receiverIntent.setAction("com.bd.action.FRIEND_LOCATIONMESSAGE_ACTION");
    ...
    private void parseLocationReport(Context context, BDMessageInfo info, String msg, byte[] mMessageArray) {
        try {
    ...
    private void parseZZTH(Context context, BDMessageInfo info, String msg) {
        try {
    ...
    private void parseBDNAC(Context context, BDMessageInfo info, String msg) {
        ...
        try {
            ...
            if (id > 0) {
                ...
                builder.setTicker(info.getmUserAddress());
                builder.setContentTitle(new String("来自 " + info.getmUserAddress() + " 信息"));
                builder.setContentText("接收到从指挥机上发送的指令导航,是否进入导航?");
                ...
                mNotificationManager.notify(Config.BDNAC_NOTIFICATION, builder.build());
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void parseBDNAL(Context context, BDMessageInfo info, String msg) {
        String s = CollectionUtils.trimEnter(msg);
        msg = s;
    ...
    private void notificationNaviLine(BDMessageInfo info, BDLineNavOperation navOper, String lineId) {
        ...
        mNotificationManager.notify(Config.BDNAL_NOTIFICATION, builder.build());
    }
...
//        }
    ...
    private void storeMsg4G(Context context, BDMessageInfo info, String msg) {
        long rowId = saveMessage4GToDataBase(context, info, msg);
        Utils.BD_MESSAGE_PAGER_INDEX = 2;
        //发送广播
        Intent receiverIntent = new Intent();
        receiverIntent.putExtra("ROWID", rowId);
    ...
    private long saveMessageToDataBase(Context context, BDMessageInfo info, String messagContent) {
        ...
        DatabaseOperation oper = DatabaseOperation.getInstance();
    ...
    private long saveMessage4GToDataBase(Context context, BDMessageInfo info, String messagContent) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String date = sdf.format(new Date());
        DatabaseOperation oper = DatabaseOperation.getInstance();
        BDMSG bDMsg = new BDMSG();
        ...
        bDMsg.setColumnsCrc("");
        bDMsg.setColumnsMsgFlag("3");
        long rowId = oper.insert4G(bDMsg);
        return rowId;
    }
