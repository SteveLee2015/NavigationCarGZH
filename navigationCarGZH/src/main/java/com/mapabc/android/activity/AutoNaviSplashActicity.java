...
//						+ msg.obj);
				...
				int newintversion = ToolsUtils.getIntVersion(version);
				int currenversion = ToolsUtils.getIntVersion(currentversion[1]
						.split("\\_")[0]);
				if (newintversion > currenversion) {
					showUpdateTip(version, Integer.parseInt(versionarray[1]));
				} else {
					ToolsUtils.showTipInfo(AutoNaviSplashActicity.this,
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TAG,"==============================>path="+(SysParameterManager.getBasePath() + "/MapABC"));
        TextView naviSplashTitleTextView=(TextView) this.findViewById(R.id.navisplash_title_textview);
        naviSplashTitleTextView.setTypeface(ToolsUtils.getFontStyle(this));
        TextView naviSplashVersionTextView =(TextView) this.findViewById(R.id.navisplash_version_textview);
        naviSplashVersionTextView.setTypeface(ToolsUtils.getFontStyle(this));
        naviSplashVersionTextView.setText("v"+ToolsUtils.getAppVersion(this));
        TextView naviSplashPromptTextView=(TextView) this.findViewById(R.id.navisplash_prompt_textview);
        naviSplashPromptTextView.setTypeface(ToolsUtils.getFontStyle(this));
		// 判断SDCard是否存在
		...
		//	showDisclaimer();
		//} else
        //if (SettingForLikeTools.getUpdateState(this) == 0) {
			// 联网判断当前版本
			//getversion();
		//} else {
			showLogo();
		...
		if (new File(SysParameterManager.getBasePath()).exists()) {
			String path = "/mnt/extsd/MapABC/log";
	...
	private void getversion() {
		String url = ToolsUtils.getUpdateSysURL();
