 ...
 *
 */
public class SearchUserEyeActivity extends BottomBaseActivity {
    ...
    private void init(){
    	if(!getPOIData(1)){
			ToolsUtils.showTipInfo(SearchUserEyeActivity.this, R.string.common_no_data);
			this.finish();
			return ;
		}
			...
			if(pageResults==null||pageResults.userEyes==null||pageResults.userEyes.length<1){
			...
			}
				{
					for (UserEventPot data: pageResults.userEyes) {
						...
						poi.name = data.name;
						String district = ToolsUtils.getDistrictByAdCode(poi, SearchUserEyeActivity.this);
...
//	}
	...
	private void initTop(){
		...
		title_name.setText(this.getResources().getStringArray(R.array.searchlocitems)[7]);
