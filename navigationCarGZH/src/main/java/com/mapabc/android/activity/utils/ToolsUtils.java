...
//		intent.setClass(RouteManagerActivity.this, com.mapabc.android.activity.NaviStudioActivity.class);
    ...
    public static void showPoiTip(MapView m_mapView, Context context, float x, float y, String name) {
        ...
        //tvTipRoadNam.setTextColor(0xFFFFFF);
...
//		poi.cFavoriteADCode = centerinfo.cADCode;
        String roadName = ToolsUtils.getRoadName(centerinfo, 50);
        if (roadName.equals("")) {
            roadName = "未知道路";
        }
    ...
    public static Animation setShowAnimation(Context context) {
        ...
        mShowAction.setInterpolator(AnimationUtils.loadInterpolator(context, android.R.anim.bounce_interpolator));
        return mShowAction;
    }
    ...
    public static Typeface getFontStyle(Context mContext) {
        if (typeface == null) {
            typeface = Typeface.createFromAsset(mContext.getAssets(), "fonts/SOFTBLACK.TTF");
        }
        return typeface;
    }
    ...
    public static double[] wgs84togcj02(double wgs_lon, double wgs_lat) {
        if (out_of_china(wgs_lon, wgs_lat)) {
            ...
            double radwgs_lat = wgs_lat / 180.0 * Math.PI;
            double magic = Math.sin(radwgs_lat);
            magic = 1 - ee * magic * magic;
            double sqrtmagic = Math.sqrt(magic);
            dwgs_lon = (dwgs_lon * 180.0) / (a / sqrtmagic * Math.cos(radwgs_lat) * Math.PI);
            dwgs_lat = (dwgs_lat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * Math.PI);
