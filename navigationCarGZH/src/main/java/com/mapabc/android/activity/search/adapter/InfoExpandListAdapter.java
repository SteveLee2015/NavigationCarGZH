...
public class InfoExpandListAdapter extends BaseExpandableListAdapter {
	...
	public View getGroupView(final int groupPosition, final boolean isExpanded,
		...
		View view;
			...
			int width = ToolsUtils.getCurScreenWidth(view.getContext());
			...
			String district = ToolsUtils.getDistrictByAdCode(data, context);
			if(ToolsUtils.isLand(view.getContext())){
			...
			if(dist < 1000){
				...
				if(temp.contains(".")){
					...
					strDist=temp.substring(0, index)+ToolsUtils.getValue(view.getContext(),R.string.searchpoiresult_unit_m);
				}else {
					strDist = temp+ToolsUtils.getValue(view.getContext(),R.string.searchpoiresult_unit_m);
				}
			}else{
				DecimalFormat df = new DecimalFormat();
				df.applyPattern("#.00");
				strDist = df.format((double) dist / 1000.0D)+ToolsUtils.getValue(view.getContext(),R.string.searchpoiresult_unit_km);
			}
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		try{
			...
			childViewHolder.addrTextView.setText(ToolsUtils.getValue(convertView.getContext(),
			...
			childViewHolder.districtTextView.setText(ToolsUtils.getValue(convertView.getContext(),
					R.string.searchpoiresult_strArea) + district);
			
			childViewHolder.telTextView.setText(ToolsUtils.getValue(convertView.getContext(), R.string.searchpoiresult_strTel)
					+ data.telephone);
