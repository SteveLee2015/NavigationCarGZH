 ...
 * 系统初始化线程
 */
public class AsyncStartForward extends AsyncTask<Void, Void, Void> {
	private static final String TAG = "AsyncForward";
	private Activity act;
	private String target;
	TTSOptions ttsOptions = new TTSOptions();
	public AsyncStartForward(Activity act,String target){
		this.act= act;
		this.target = target;
	}			
	...
	protected Void doInBackground(Void... params) {
		...
		if(act!=null){
			...
			mapOptions.screenWidth = ToolsUtils.getCurScreenWidth(act);
			mapOptions.screenHeight = ToolsUtils.getCurScreenHeight(act);
	...
	protected void onPostExecute(Void result){
		...
		if(UtilAPI.getInstance().verifyUserKey()!=0){
			showExitTip(ToolsUtils.getValue(act,R.string.asyncforward_registe_tip));
			return ;
		}
//		初始化系统参数
