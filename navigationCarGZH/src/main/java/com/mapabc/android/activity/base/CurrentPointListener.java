...
public class CurrentPointListener implements OnTouchListener{
	...
	public static final int STATUS_CAR_AT_CENTER = 0;// 中心点为车位点  3个
	public static final int STATUS_HAS_NO_ROUTE = 1;// 中心点非车位点且无路径  6个
	public static final int STATUS_HAS_ROUTE = 2;// 中心点非车位点有路径   7个
	...
	public void setFootView() {
		...
		if (isMapMoved) {
			...
			case 0:// day
				currentPointBtn
				.setBackgroundResource(R.drawable.cp_extra_day_normal);//白底黑色
				break;
			case 1://night
				currentPointBtn
				.setBackgroundResource(R.drawable.cp_extra_night_normal);//黑底白色
				break;
			default:
				break;
			}
	...
	public void openAlertDialog() {
		...
		String titleName=ToolsUtils.getRoadName(nsLonLat_mapCenter, 100);
		if(titleName.equals("")){
			titleName="未知道路";
		}
...
//								    	  break;
	 ...
	 */
	public boolean setAvoidLine(NSLonLat centerinfo){
		try{
		if(!RouteAPI.getInstance().canAddAvoidPoint()){
    		ToolsUtils.showTipInfo(activity, R.string.currentpoint_add_avoidpoint_fail);
			return false;
		}
		if(RouteAPI.getInstance().addAvoidPoint(centerinfo)){
		    ...
		    ToolsUtils.showTipInfo(activity, R.string.currentpoint_addavoidline_failed);
		    return false;
		}
		}catch(Exception ex){
			Log.e(TAG, "ERR",ex);
     ...
     * 设置家
     */
    public boolean setHomePoint(NSLonLat centerinfo){
//    	if(FavoriteAPI.getInstance().delFavorite_ALL(FavoriteConst.FAVORITE_HOME)==1){
    	if(FavoriteAPI.getInstance().delAllFavorites(Const.FAVORITE_HOME)==1){
    		...
    		ToolsUtils.getFavoriteInfo(centerinfo, favoriteInfo);
    		if(favoriteInfo.name.equals("")){
    			ToolsUtils.showTipInfo(activity, R.string.roadname_null);
    			return false;
    		}else if(favoriteInfo.longitude<0.0000001f||favoriteInfo.latitude<0.0000001f){
    			ToolsUtils.showTipInfo(activity, R.string.lonlat_null);
    			return false;
    		}
    		if(FavoriteAPI.getInstance().addFavorite(favoriteInfo)!=0){
    			ToolsUtils.showTipInfo(activity, R.string.sethome);
    		}else{
    			ToolsUtils.showTipInfo(activity, R.string.sethomefail);
    		}
    	}
    	return true;
    }
    ...
    private boolean addPOIToFavorites(NSLonLat centerinfo){
    	String numString = activity.getString(R.string.history_destination_num_prompt);
    	int num = Integer.parseInt(numString);
    	if(FavoriteAPI.getInstance().getFavoriteCount(Const.FAVORITE_HAUNT)>= num){
    		ToolsUtils.showTipInfo(activity, R.string.favorite_full_tip);
    	}else{
    		FavoriteInfo favoriteInfo = new FavoriteInfo();
    		...
    		if(activity.poiInfo!=null&&activity.poiInfo.lat==centerinfo.y&&centerinfo.x == activity.poiInfo.lon){
    			ToolsUtils.getFavoriteInfo(activity.poiInfo, favoriteInfo);
    		}else{
    			ToolsUtils.getFavoriteInfo(centerinfo, favoriteInfo);
    		}
    		Log.e(TAG, "当前点收藏："+favoriteInfo.longitude+","+favoriteInfo.latitude);
    		if(FavoriteAPI.getInstance().addFavorite(favoriteInfo)!=0){
    			ToolsUtils.showTipInfo(activity, R.string.favorite_successfully);
    		}else{
    			ToolsUtils.showTipInfo(activity, R.string.favorite_successfail);
    		}
    	}
    	return true;
    }
    ...
    public boolean addPOIToHistory(NSLonLat centerinfo){
    	...
    	if(FavoriteAPI.getInstance().getFavoriteCount(Const.FAVORITE_HISTORYDES)>=200){
    		ToolsUtils.showTipInfo(activity, R.string.favorite_full_tip1);
    	}else{
    		FavoriteInfo favoriteInfo = new FavoriteInfo();
     ...
     * 设置目的地
     */
    public boolean setDestination(NSLonLat centerinfo,int type){
    	if(activity.poiInfo!=null&&activity.poiInfo.lat==centerinfo.y&&centerinfo.x == activity.poiInfo.lon){
     ...
     * 设置途经点
     */
    public boolean setPassPoint(NSLonLat centerinfo){
    	try{
    	if(RouteAPI.getInstance().canAddPassPoint()){
    		...
    		for(int i=0;i<count;i++){
    			if(centerinfo.x == passpoints[i].x&&centerinfo.y == passpoints[i].y){
    				ToolsUtils.showTipInfo(activity, R.string.currentpoint_addsamplepasspointtip);
    				return false;
    			}
    		}
    		...
    		ToolsUtils.showTipInfo(activity, R.string.currentpoint_add_passpoint_fail);
    	}
    	}catch(Exception ex){
     ...
     */
    public void reCalculatePath(final int type){
...
//                		Log.e(TAG, "reCalculatePath7");
    ...
    private void addPOIToUserEye(NSLonLat centerinfo){
    	String titleName=ToolsUtils.getRoadName(centerinfo, 100);
		if(titleName.equals("")){
		...
		if(id!=-1){
			ToolsUtils.showTipInfo(activity, R.string.addusereye_successfully);
			userEyeInfo.id = id;
			userEyeInfo.time = "2012-09-29 12:12:12";
			boolean b = RouteAPI.getInstance().addUserEyeData(userEyeInfo);
			System.out.println("********"+b);
		}else{
			ToolsUtils.showTipInfo(activity, R.string.addusereye_successfail);
		}
    }
    ...
    public boolean addPOIToHistory(SearchResultInfo poi){
    	...
    	if(FavoriteAPI.getInstance().getFavoriteCount(Const.FAVORITE_HISTORYDES)>=200){
    		...
    		FavoriteInfo favoriteInfo = new FavoriteInfo();
    		favoriteInfo.type = Const.FAVORITE_HISTORYDES;
    		ToolsUtils.getFavoriteInfo(poi, favoriteInfo);
    		int res = FavoriteAPI.getInstance().addFavorite(favoriteInfo);
    		Log.e(TAG, "RES:"+res);
    	}
    	Log.e(TAG, "AddPOIToHistory end");
