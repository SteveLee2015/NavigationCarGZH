...
public class BDCommManager {
    ...
    private ArrayList<BDPlatformFKListener> mBDPlatformFKListeners = new ArrayList<>();
    private ArrayList<BDSOSButtonListener> mSOSButtonListeners = new ArrayList<>();
    private ArrayList<Network4GListener> mNetwork4GListeers = new ArrayList<>();

    private Map<Integer, Boolean> mBDFixNumberMap = new HashMap<>();
    private Map<Integer, Boolean> mGPFixNumberMap = new HashMap<>();

    private List<BDSatellite> bdlist = null;
    private List<GPSatellite> gplist = null;

    private LocDetailInfo bdDetailInfo = new LocDetailInfo(),
            gpDetailInfo = new LocDetailInfo(),
            gnDetailInfo = new LocDetailInfo();
            ...
            if ("S500".equals(DEVICE_MODEL)) {
                ...
                for (int i = 0; i < listener.length; i++) {
                    ...
                    if (mBDLocationListeners.contains(listener[i])
                            ...
                            || mBDPlatformFKListeners.contains(listener[i])
                            || mSOSButtonListeners.contains(listener[i])
                            || mNetwork4GListeers.contains(listener[i])) {
                        continue;
                    }
                    ...
                    } else if (listener[i] instanceof BDPlatformFKListener) {
                        mBDPlatformFKListeners.add((BDPlatformFKListener) listener[i]);
                    } else if (listener[i] instanceof BDSOSButtonListener) {
                        mSOSButtonListeners.add((BDSOSButtonListener) listener[i]);
                    } else if (listener[i] instanceof Network4GListener) {
                        mNetwork4GListeers.add((Network4GListener) listener[i]);
                    } else {
            ...
            if ("S500".equals(DEVICE_MODEL)) {
                ...
                for (int i = 0; i < listener.length; i++) {
                    ...
                    if (listener[i] instanceof BDLocationListener) {
                        ...
                        mBDPlatformFKListeners.remove(listener[i]);
                    } else if (listener[i] instanceof BDSOSButtonListener) {
                        mSOSButtonListeners.remove(listener[i]);
                    } else if (listener[i] instanceof Network4GListener) {
                        mNetwork4GListeers.remove(listener[i]);
                    } else {
        ...
        if ("S500".equals(DEVICE_MODEL)) {
            ...
            try {
                ...
                if (serialport.isEmpty()) {
                    ...
                    if ("ttyS1".equals(serialport)) {
                        mSerialPort = mApplication.getSerialPort("/dev/ttyS1", IntBaudrate);
                    ...
                    } else if ("ttyS7".equals(serialport)) {
                        mSerialPort = mApplication.getSerialPort("/dev/ttyS7", IntBaudrate);
                        Log.e(TAG, "配置串口为:/dev/ttyS7  波特率为:IntBaudrate");
                    } else {
                        //默认连接串口0
    ...
    Runnable onetimeThread = new Runnable() {
        @Override
        public void run() {
            mReadThread = new ReadThread();
            mReadThread.start();
    ...
    public void sendLocationInfoReqCmdBDV21(boolean isImmediate,
        ...
        if ("S500".equals(DEVICE_MODEL)) {
            ...
            try {
                ...
                }/* 1-无高程 */ else if (heightType == 1) {
                    heightValue = 0d;
                }/* 2-测高1 */ else if (heightType == 2) {
                    heightValue = 0d;
                }/* 3-测高2 */ else if (heightType == 3) {
                    if (isShowHeight) {// 高空
                        /* 高程数据位空 */
                        heightValue = 0d;
                    }
        ...
        if ("S500".equals(DEVICE_MODEL)) {
            ...
            try {
                ...
                if (encodeMode == 0 || encodeMode == 1) {
                    ...
                    temp[2] = (byte) 0xF2;
                    temp[3] = (byte) 0x03;
                    System.arraycopy(array, 0, temp, 4, array.length);
    ...
    public void sendSMSCmdBDStdV21(String receiveAddress, int commType,
                                   int encodeMode, String responsemode, String message) throws BDUnknownException, BDParameterException {
        ...
        if ("S500".equals(DEVICE_MODEL)) {
            ...
            try {
                StringBuffer cmd = new StringBuffer();
                cmd.append("$CCTXA,").append(receiveAddress).append(",")
                        .append(commType).append(",").append(encodeMode).append(",");
                /* 汉字或代码 */
                byte[] msgBuffer = null;
                if (encodeMode == 0 || encodeMode == 1) {
                    msgBuffer = message.getBytes("GBK");
                }/* 混合传输 */ else if (encodeMode == 2) {
                    byte[] array = message.getBytes("GBK");
                    msgBuffer = new byte[array.length + 1];
                    msgBuffer[0] = (byte) 0xA4;
                    System.arraycopy(array, 0, msgBuffer, 1, array.length);
                    String hex = bytesToHexString2(msgBuffer);
                    msgBuffer = hex.getBytes("GBK");
                }
                write(bd2SendSMSPackage(cmd.toString(), msgBuffer));
     ...
     * 设置SOS配置
     */
    public void sendSOSSettingsCmd(String id, int encodeMode, String settings)
            throws BDUnknownException, BDParameterException {
        if ("S500".equals(DEVICE_MODEL)) {
            ...
            if (TextUtils.isEmpty(id)) {
                throw new BDParameterException(
                        "android.location.BDParameterException: sendSOSSettingsCmd() 'String id' parameter exception");
            }
            if ((encodeMode != 2) && (encodeMode != 1) && (encodeMode != 3)) {
                throw new BDParameterException(
                        "android.location.BDParameterException: sendSOSSettingsCmd() 'int encodeMode' parameter exception");
            }
            if (TextUtils.isEmpty(settings)) {
                throw new BDParameterException(
                        "android.location.BDParameterException: sendSOSSettingsCmd() 'String settings' parameter exception");
            }
            try {
                String cmd = id + "," + encodeMode + "," + settings;
                write(bd2SendPackage("$CCPTS", cmd));
            } catch (Exception e) {
                Log.e("BDRDSS",
                        "android.location.BDUnknownException:sendSOSSettingsCmd()",
                        e);
                throw new BDUnknownException(
                        "android.location.BDUnknownException:sendSOSSettingsCmd()",
                        e);
    ...
    public void sendNetwork4GCmd(String cmd) throws BDUnknownException, BDParameterException {
        if ("S500".equals(DEVICE_MODEL)) {

        } else {
            try {
                write(bd2SendPackage("$CCATC", cmd));
            } catch (Exception e) {
                Log.e("BDRDSS",
                        "android.location.BDUnknownException:sendNetwork4GCmd()",
                        e);
                throw new BDUnknownException(
                        "android.location.BDUnknownException:sendNetwork4GCmd()",
                        e);
    ...
    public void sendNetwork4GCmd(byte[] cmd) throws BDUnknownException, BDParameterException {
        if ("S500".equals(DEVICE_MODEL)) {

        } else {
            try {
                write(bd2SendSMSPackage("$CCATC,", cmd));
            } catch (Exception e) {
                Log.e("BDRDSS",
                        "android.location.BDUnknownException:sendNetwork4GCmd()",
                        e);
        ...
        public void run() {
            ...
            while (!isInterrupted()) {
                ...
                try {
                    ...
                    if (size > 0) {
//                        if (size - 2 > 0) {
//                            cmd1 = new String(buffer, 0, size - 2, "GBK");
//                            Log.i(TAG, "=================>cmd1 = " + cmd1);
//                        }
                        try {
                            sleep(5);
...
//				mContext.sendBroadcast(intent);
            ...
            //$BDTXR,3,0455998,2,,A4CDA8D0C5B2E2CAD420D2BBB6FEC8FDCBC4CEE5C1F9C6DFB0CBBEC5CAAE31323334353637383930*3
                ...
                boolean a4A0F4 = false;
                String smdCmd = new String(buffer, 0, size - 2, "GBK");
                Log.i("TEST", "========================>smdCmd= " + smdCmd);
                BDMessageInfo mBDMessageInfo = new BDMessageInfo();
                String message = "";
                String[] results = smdCmd.split(",");
                ...
                if (Integer.valueOf(encodingMode) == 2) {
                    ...
                    messageArr = hexStringToBytes(msg.substring(2));
                    message = new String(messageArr, "GBK");
                } else {
                    /* 当编码方式是代码或汉字方式 */
                    messageArr = new byte[count];
                    System.arraycopy(buffer, startCount, messageArr, 0, messageArr.length);
                    message = new String(messageArr, "GBK");
                }

                mBDMessageInfo.setmUserAddress(userAddress);
                mBDMessageInfo.setMsgCharset(Integer.valueOf(encodingMode));
                ...
                if (a4A0F4) {
                    String hexCmd = bytesToHexString2(messageArr);
                    Log.i("TEST", "=================================>hexCmd =" + hexCmd);
                    String mFlag = hexCmd.substring(4, 6);
                    for (BDPlatformFKListener listener : mBDPlatformFKListeners) {
                        listener.onReceipt(mFlag);
                    }
                } else {
...
//				mContext.sendBroadcast(intent);
                ...
                if (mBDRNSSLocationListeners != null) {
                    ...
                    if (!gnDetailInfo.isEmpty()) {
                        gnLocation.setTime(gnDetailInfo.getTime());
                        gnLocation.setSpeed(gnDetailInfo.getSpeed());
                        gnLocation.setBearing(gnDetailInfo.getBearing());
                        gnLocation.setAvailable(gnDetailInfo.isAvailable());

                        Log.i(TAG, gnLocation.getLongitude() + ","
                                + gnLocation.getLatitude() + ","
                                + gnLocation.getAltitude() + ","
                                + gnLocation.getTime() + ","
                                + gnLocation.getBearing() + ","
                                + gnLocation.getSpeed() + ""
                                + gnLocation.isAvailable());
                ...
                if (mBDRNSSLocationListeners != null) {
                    ...
                    float bearing = Float.valueOf(bdRNSSRMCArray[8]);
                    float speed = (float) (Float.valueOf(bdRNSSRMCArray[7]) * 1.852 / 3.6);

                    long locTime = 0;
                    try {
                        locTime = sdf.parse(getTimeFormat(bdRNSSRMCArray[9], bdRNSSRMCArray[1])).getTime();
                    } catch (ParseException e2) {
                        e2.printStackTrace();
                    }
                    gnDetailInfo.setParams(bdRNSSRMCArray[2].equals("A"), locTime, speed, bearing);

                    if (this.gnLocation != null) {
                        this.gnLocation.setAvailable(gnDetailInfo.isAvailable());
                        // 速度为  节 *1.852 为 km/h  /3.6为  m/s
                        this.gnLocation.setSpeed(gnDetailInfo.getSpeed());
                        this.gnLocation.setBearing(gnDetailInfo.getBearing());
                        this.gnLocation.setTime(gnDetailInfo.getTime());
                ...
                if (mBDRNSSLocationListeners != null) {
                    ...
                    if (!bdDetailInfo.isEmpty()) {

                        bdLocation.setTime(bdDetailInfo.getTime());
                        bdLocation.setSpeed(bdDetailInfo.getSpeed());
                        bdLocation.setBearing(bdDetailInfo.getBearing());
                        bdLocation.setAvailable(bdDetailInfo.isAvailable());

                        Log.i(TAG, bdLocation.getLongitude() + ","
                                + bdLocation.getLatitude() + ","
                                + bdLocation.getAltitude() + ","
                                + bdLocation.getTime() + ","
                                + bdLocation.getBearing() + ","
                                + bdLocation.getSpeed() + ""
                                + bdLocation.isAvailable());

                        for (BDRNSSLocationListener mBDRNSSLocationListener : mBDRNSSLocationListeners) {
                            mBDRNSSLocationListener.onLocationChanged(bdLocation);
                        }
                        bdDetailInfo.clearParams();
                        bdLocation = null;
                    }
                } else {
                    Log.i(TAG, "mBDRNSSLocationListeners is null!");
                ...
                if (mBDRNSSLocationListeners != null) {
                    ...
                    bdDetailInfo.setParams(bdRNSSRMCArray[2].equals("A"), locTime, speed, bearing);
                ...
                if (mBDRNSSLocationListeners != null) {
                    ...
                    if (!gpDetailInfo.isEmpty()) {

                        gpLocation.setAvailable(gpDetailInfo.isAvailable());
                        gpLocation.setSpeed(gpDetailInfo.getSpeed());
                        gpLocation.setBearing(gpDetailInfo.getBearing());
                        gpLocation.setTime(gpDetailInfo.getTime());

                        Log.i(TAG, gpLocation.getLongitude() + ","
                                + gpLocation.getLatitude() + ","
                                + gpLocation.getAltitude() + ","
                                + gpLocation.getTime() + ","
                                + gpLocation.getBearing() + ","
                                + gpLocation.getSpeed() + ""
                                + gpLocation.isAvailable());

                        for (BDRNSSLocationListener mBDRNSSLocationListener : mBDRNSSLocationListeners) {
                            mBDRNSSLocationListener.onLocationChanged(gpLocation);
                        }
                        gpDetailInfo.clearParams();
                        gpLocation = null;
                    }
                } else {
                    Log.i(TAG, "mBDRNSSLocationListeners is null!");
                ...
                if (mBDRNSSLocationListeners != null) {
                    String[] gpRNSSRMCArray = cmd.split(",");

                    float speed = (float) (Float.valueOf(gpRNSSRMCArray[7]) * 1.852 / 3.6);
                    float bear = Float.valueOf(gpRNSSRMCArray[8]);

                    long locTime = 0;
                    try {
                        locTime = sdf.parse(getTimeFormat(gpRNSSRMCArray[9], gpRNSSRMCArray[1])).getTime();
                    } catch (ParseException e2) {
                        e2.printStackTrace();
                    }

                    gpDetailInfo.setParams(gpRNSSRMCArray[2].equals("A"), locTime, speed, bear);
...
//						}
                        ...
                        if (mBDFixNumberMap.containsKey(mBDSatellite.getPrn() - 160) && mBDFixNumberMap.get(mBDSatellite.getPrn() - 160)) {
                            mBDSatellite.setUsedInFix(true);
                            mBDFixNumberMap.put(mBDSatellite.getPrn() - 160, false);
                        } else {
                ...
                if (mGPSatelliteListeners != null) {
                    ...
                    for (int i = 0; i < num; i++) {
                        ...
                        if (mGPFixNumberMap.containsKey(mGPSatellite.getPrn()) && mGPFixNumberMap.get(mGPSatellite.getPrn())) {
                            mGPSatellite.setUsedInFix(true);
                            mGPFixNumberMap.put(mGPSatellite.getPrn(), false);
                        } else {
                ...
                for (int i = 3; i <= 14; i++) {
                    ...
                    if (prn < 160)
                        prn += 160;
                    mBDFixNumberMap.put(prn - 160, true);
                }
            ...
            } else if (cmd.startsWith("$BDPTS")) {
                if (mSOSButtonListeners != null) {
                    String[] bdpts = cmd.split(",");
                    String manager = bdpts[1].split("\\*")[0];
                    int value = 0;
                    if (!TextUtils.isEmpty(manager) && TextUtils.isDigitsOnly(manager)) {
                        value = Integer.valueOf(manager);
                    }
                    for (BDSOSButtonListener bdsosButtonListener : mSOSButtonListeners) {
                        bdsosButtonListener.onSosReceipt(value);
                    }
                }
            } else if (cmd.startsWith("$BDATR")) {
                if (mNetwork4GListeers != null) {
                    String[] bdpts = cmd.split(",");
                    String manager = bdpts[1].split("\\*")[0];
                    if (!manager.startsWith("+") && manager.length() > 8) {
                        BDMessageInfo mBDMessageInfo = new BDMessageInfo();
                        if (!TextUtils.isEmpty(manager)) {
                            String sms = paraseSMSInfo(manager);
                            Log.i("TEST", "===============================>" + sms);
                            if (!TextUtils.isEmpty(sms)) {
                                String[] infoArr = sms.split("\\:");
                                mBDMessageInfo.setmUserAddress(infoArr[0]);
                                mBDMessageInfo.setMessage(("CMGS" + infoArr[1]).getBytes("GBK"));
                            }
                        }
                        Intent intent = new Intent("abc.abc");
                        intent.putExtra(BDCommManager.BDRDSS_MESSAGE, mBDMessageInfo);
                        ...
                        for (Network4GListener network4GListener : mNetwork4GListeers) {
                            network4GListener.on4GResponse(manager);
                        }
                    }
                }
            }
     ...
     * @return
     */
    public static String paraseSMSInfo(String pduPayload) throws UnsupportedEncodingException {
        if (TextUtils.isEmpty(pduPayload)) {
            return "";
        }
        int smsCenterLength = Integer.valueOf(pduPayload.substring(0, 2)) - 1;
        StringBuffer smsCenter = new StringBuffer();
        smsCenter.append("+");
        for (int index = 0; index < smsCenterLength; index++) {
            smsCenter.append(pduPayload.substring(index * 2 + 5, index * 2 + 6)).append(pduPayload.substring(index * 2 + 4, index * 2 + 5));
        }
        Log.i("TEST", "=======================>smsCenterPhone =" + smsCenter.toString());

        int senderPhoneLength = getValueByStr(pduPayload.substring(smsCenterLength * 2 + 6, smsCenterLength * 2 + 8)) - 1;
        StringBuffer senderPhone = new StringBuffer();
        senderPhone.append("+");
        for (int index = 0; index < 7; index++) {
            senderPhone.append(pduPayload.substring(smsCenterLength * 2 + index * 2 + 11, smsCenterLength * 2 + index * 2 + 12)).append(pduPayload.substring(smsCenterLength * 2 + index * 2 + 10, smsCenterLength * 2 + index * 2 + 11));
        }
        String sendPhoneNumber =  senderPhone.toString().substring(0 ,senderPhone.toString().length() -1);
        Log.i("TEST", "=======================>senderPhone =" + senderPhone.toString() + ",sendPhoneNumber =" +sendPhoneNumber);

        int smsLength = getValueByStr(pduPayload.substring(smsCenterLength * 2 + senderPhoneLength * 2 + 18, smsCenterLength * 2 + senderPhoneLength * 2 + 20));

        Log.i("TEST", "====================>smsLength =" + smsLength);
        String message = pduPayload.substring(smsCenterLength * 2 + senderPhoneLength * 2 + 20);
        Log.i("TEST", "====================>smsLength =" + message);
//        StringBuffer msgBf = new StringBuffer();
//        for (int index = 0; index < smsLength * 2 / 4; index++) {
//            msgBf.append("\\u");
//            msgBf.append(message.substring(index, index + 4));
//        }
//        Log.i("TEST", "======================>" + msgBf.toString());
        return sendPhoneNumber + ":" + new String(hexStringToBytes(message) , "GBK");
    }
    ...
    public static int getValueByStr(String valueStr) {
        ...
        if (isOctNumberRex(valueStr)) {
            return Integer.valueOf(valueStr);
        } else if (isHexNumberRex(valueStr)){
...
//        }
    ...
    @NonNull
    private String getTimeFormat(String yearMothDay, String hourMSecond) {
        String dateStr = "";
        if (!TextUtils.isEmpty(yearMothDay) && yearMothDay.length() == 6) {
            String day = yearMothDay.substring(0, 2);
            String month = yearMothDay.substring(2, 4);
            String year = "20" + yearMothDay.substring(4, 6);
            dateStr = year + "" + month + "" + day + " " + hourMSecond;
        }
        return dateStr;
    }
